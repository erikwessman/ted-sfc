...
def reduce_video(src_file, dst_file, ratio, frame_ids):
    ...
    ind = 0
    # dest capture
    dst_size = (int(frame.shape[1] * ratio),
                int(frame.shape[0] * ratio))  # (width, height)
    cap_dst = cv2.VideoWriter(
        dst_file, cv2.VideoWriter_fourcc(*'XVID'), 30, dst_size)
    while (ret):
        if ind in frame_ids:
...
def reduce_data(data_path, ratio, max_frames, subset, result_path):
    ...
    for accID in sorted(os.listdir(coord_path_src)):
        ...
        for filename in sorted(os.listdir(txtfile_dir)):
            ...
            print("Processing the video: %s/%s, # frames: %d" %
                  (accID, vid, len(frame_ids)))
            # resize & write coords
            ...
            # read salmap videos
            salmap_video_src = os.path.join(
                salmap_path_src, accID, vid + '.avi')
            salmap_video_dst = os.path.join(
                salmap_path_dst, accID, vid + '.avi')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Reduce the size of DADA-2000')
    ...
    reduce_data(args.data_path, ratio, max_frames,
                'training', args.result_path)
    reduce_data(args.data_path, ratio, max_frames, 'testing', args.result_path)
    reduce_data(args.data_path, ratio, max_frames,
                'validation', args.result_path)
