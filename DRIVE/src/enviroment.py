...
class DashCamEnv(core.Env):
    def __init__(self, cfg, device=torch.device("cuda")):
        ...
        self.foveal_model = TorchFovea(cfg.input_shape, min(
            cfg.input_shape)/6.0, level=5, factor=2, device=device)
    ...
    def set_model(self, pretrained=False, weight_file=None):
        if pretrained and weight_file is not None:
            ...
            assert os.path.exists(
                weight_file), "Checkpoint directory does not exist! %s" % (weight_file)
    ...
    def set_data(self, video_data, coord_data, data_info):
        ...
        self.video_data = video_data.float().to(
            self.device, non_blocking=True)  # (B, T, 3, H, W)
        self.coord_data = coord_data.float().to(
            self.device, non_blocking=True)  # (B, T, 2): (x,y)

        # maximum number of steps
        self.max_steps = (self.video_data.size(
            1) - self.len_clip + 1) // self.step_size
        # neg/pos labels
        self.clsID = data_info[:, 4].to(self.device)
        # time-of-accident (seconds), for neg: toa=-1
        self.begin_accident = data_info[:, 5].to(self.device) / float(self.fps)
    ...
    def reset(self):
        ...
        # fetch video clip
        frame_data = self.video_data[:, self.cur_step*self.step_size: self.cur_step *
                                     self.step_size+self.len_clip]  # (B, T, C, H, W)
        # observation by computing saliency
        with torch.no_grad():
            self.cur_state = self.observe(
                frame_data, fixation=None)  # GPU Array(B, 124)
    ...
    def minmax_norm(self, salmap):
        ...
        salmap: (B, 1, H, W)
        """
        batch_size, height, width = salmap.size(
            0), salmap.size(2), salmap.size(3)
    ...
    def observe(self, data_input, fixation=None):
        ...
        if self.saliency == 'MLNet':
            ...
            saliency_bu, bottom = self.observe_model(
                data_input, return_bottom=True)  # (B, 1, H, W)
            ...
            if fixation is not None:
                ...
                # saliency fusion
                rho = self.rho if self.fusion == 'static' else self.rho.unsqueeze(
                    1).unsqueeze(1).unsqueeze(1)  # (B, 1, 1, 1)
                saliency = (1 - rho) * saliency_bu + rho * saliency_td
            else:
                saliency = saliency_bu.clone()
            # construct states
            state_max = F.max_pool2d(
                saliency * bottom, kernel_size=bottom.size()[2:]).squeeze_(dim=-1).squeeze_(dim=-1)
            state_avg = F.avg_pool2d(
                saliency * bottom, kernel_size=bottom.size()[2:]).squeeze_(dim=-1).squeeze_(dim=-1)
        elif self.saliency == 'TASED-Net':
            data_observe = data_input.permute(
                0, 2, 1, 3, 4).contiguous()  # (B, C=3, T=32, H=480, W=640)
            # compute saliency map
            saliency_bu, bottom = self.observe_model(
                data_observe, return_bottom=True)  # (1,1,1,60,80), [1, 192, 4, 60, 80]
            saliency_bu = self.minmax_norm(saliency_bu.squeeze(2)).unsqueeze(2)
            # compute top-down saliency map
            if fixation is not None:
                data_foveal = self.foveal_model.foveate(
                    data_input[:, 0], fixation)
                saliency_td = self.observe_model(data_foveal.unsqueeze(
                    2).repeat(1, 1, T, 1, 1))  # (B, 1, 1, 60, 80)
                saliency_td = self.minmax_norm(
                    saliency_td.squeeze(2)).unsqueeze(2)
                # saliency fusion
                rho = self.rho if self.fusion == 'static' else self.rho.unsqueeze(
                    1).unsqueeze(1).unsqueeze(1).unsqueeze(1)  # (B, 1, 1, 1, 1)
                saliency = (1 - rho) * saliency_bu + rho * saliency_td
            else:
                saliency = saliency_bu.clone()
            # construct states
            state_max = F.max_pool3d(saliency * bottom, kernel_size=bottom.size()[
                                     2:]).squeeze_(dim=-1).squeeze_(dim=-1).squeeze_(dim=-1)
            state_avg = F.avg_pool3d(saliency * bottom, kernel_size=bottom.size()[
                                     2:]).squeeze_(dim=-1).squeeze_(dim=-1).squeeze_(dim=-1)
        else:
            raise NotImplementedError
    ...
    def get_next_state(self, fixation_next, next_step):
        """mask_pred: (B, 2)
        """
        frame_data = self.video_data[:, next_step*self.step_size: next_step *
                                     self.step_size+self.len_clip]  # (B, T, C, H, W)
        with torch.no_grad():
    ...
    def get_reward(self, score_pred, fix_pred):
        ...
        exp_term = torch.clamp(self.begin_accident - self.cur_step *
                               self.step_size / self.fps, min=0)  # max(0, ta-t)
        tta_weights = (torch.exp(exp_term) - 1.0) / \
            (torch.exp(self.begin_accident) - 1.0)
        ...
        # (B, 2), [x, y]
        fix_gt = self.coord_data[:, (self.cur_step + 1)*self.step_size, :]
        # compute distance
        dist_sq = torch.sum(torch.pow(norm_fix(fix_pred, self.input_size) - norm_fix(
            fix_gt, self.input_size), 2), dim=1, keepdim=True)  # [0, sqrt(2)]
        mask_reward = fix_gt[:, 0].bool().float(
        ) * fix_gt[:, 1].bool().float()  # (B,)
        # If fixation exists (r>0 & c>0), reward = exp(-mse)
        r_atten = mask_reward.unsqueeze(1) * torch.exp(-10.0 * dist_sq)
    ...
    def step(self, actions, isTraining=True):
        ...
        fix_pred = scales_to_point(
            actions[:, 1:], self.image_size, self.input_size)  # (B, 2)  (x,y)

        # update rho (dynamic)
        if self.fusion == 'dynamic':
            self.rho = torch.clamp_max(
                score_pred.clone(), self.fusion_margin)  # (B,)
        ...
        if self.cur_step < self.max_steps - 1:  # cur_step starts from 0
            ...
            # reward (immediate)
            cur_rewards = self.get_reward(
                score_pred, fix_pred) if isTraining else 0
        else:
            # The last step
            next_state = self.cur_state.clone()  # GPU array
            cur_rewards = torch.zeros([batch_size, 1], dtype=torch.float32).to(
                self.device) if isTraining else 0
