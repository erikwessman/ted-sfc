...
def setup_dataloader(cfg, num_workers=0, isTraining=True):
    ...
    # testing dataset
    if not isTraining:
        test_data = DADA2KS(cfg.data_path, 'testing', interval=cfg.frame_interval,
                            transforms=transform_dict, use_salmap=cfg.use_salmap)
        testdata_loader = DataLoader(dataset=test_data, batch_size=cfg.batch_size,
                                     shuffle=False, drop_last=True, num_workers=num_workers, pin_memory=True)
    ...
    train_data = DADA2KS(cfg.data_path, 'training', interval=cfg.frame_interval,
                         transforms=transform_dict, use_salmap=cfg.use_salmap, data_aug=cfg.data_aug)
    traindata_loader = DataLoader(dataset=train_data, batch_size=cfg.batch_size,
    ...
    eval_data = DADA2KS(cfg.data_path, 'validation', interval=cfg.frame_interval,
                        transforms=transform_dict, use_salmap=cfg.use_salmap, data_aug=cfg.data_aug)
    evaldata_loader = DataLoader(dataset=eval_data, batch_size=cfg.batch_size,
                                 shuffle=False, drop_last=True, num_workers=num_workers, pin_memory=True)
...
def train_per_epoch(traindata_loader, env, agent, cfg, writer, epoch, memory, updates):
    ...
    for i, (video_data, _, coord_data, data_info) in tqdm(enumerate(traindata_loader), total=len(traindata_loader),
        ...
        done = torch.ones((cfg.ENV.batch_size, 1),
        ...
        while episode_steps < env.max_steps:
            ...
            cur_time = torch.FloatTensor(
                [(env.cur_step-1) * env.step_size / env.fps] * cfg.ENV.batch_size).unsqueeze(1).to(cfg.device)  # (B, 1)
            next_step = env.cur_step if episode_steps != env.max_steps else env.cur_step - 1
            gt_fix_next = env.coord_data[:,
                                         next_step * env.step_size, :]  # (B, 2)
            labels = torch.cat((cur_time, env.clsID.float().unsqueeze(
                1), env.begin_accident.unsqueeze(1), gt_fix_next), dim=1)

            mask = done if episode_steps == env.max_steps else done - 1.0
            memory.push(state, actions, rewards, next_state,
                        rnn_state, labels, mask)  # Append transition to memory
            # shift to next state
...
def eval_per_epoch(evaldata_loader, env, agent, cfg, writer, epoch):
    ...
    for i, (video_data, _, coord_data, data_info) in tqdm(enumerate(evaldata_loader), total=len(evaldata_loader),
        ...
        while episode_steps < env.max_steps:
            # select action
            actions, rnn_state = agent.select_action(
                state, rnn_state, evaluate=True)
            # step
...
def train():
    ...
    # initialize dataset
    traindata_loader, evaldata_loader = setup_dataloader(
        cfg.ENV, cfg.num_workers)

    # AgentENV
    agent = SAC(cfg.SAC, device=cfg.device)

    # Memory
    memory = ReplayMemory(cfg.SAC.replay_size) if not cfg.SAC.gpu_replay else ReplayMemoryGPU(
        cfg.SAC, cfg.ENV.batch_size, cfg.gpu_id, device=cfg.device)

    updates = 0
    for e in range(cfg.num_epoch):
        # train each epoch
        agent.set_status('train')
        updates = train_per_epoch(
            traindata_loader, env, agent, cfg, writer, e, memory, updates)
...
def test_all(testdata_loader, env, agent):
    ...
    for i, (video_data, _, coord_data, data_info) in enumerate(testdata_loader):  # (B, T, H, W, C)
        ...
        rnn_state = (torch.zeros((cfg.ENV.batch_size, cfg.SAC.hidden_size), dtype=torch.float32).to(cfg.device),
                     torch.zeros((cfg.ENV.batch_size, cfg.SAC.hidden_size), dtype=torch.float32).to(cfg.device))
        score_pred = np.zeros(
            (cfg.ENV.batch_size, env.max_steps), dtype=np.float32)
        fixation_pred = np.zeros(
            (cfg.ENV.batch_size, env.max_steps, 2), dtype=np.float32)
        fixation_gt = np.zeros(
            (cfg.ENV.batch_size, env.max_steps, 2), dtype=np.float32)
        i_steps = 0
        while i_steps < env.max_steps:
            ...
            # gather actions
            # shape=(B,)
            score_pred[:, i_steps] = info['pred_score'].cpu().numpy()
            # shape=(B, 2)
            fixation_pred[:, i_steps] = info['pred_fixation'].cpu().numpy()
            next_step = env.cur_step if i_steps != env.max_steps - 1 else env.cur_step - 1
            fixation_gt[:, i_steps] = env.coord_data[:,
                                                     next_step*env.step_size, :].cpu().numpy()
            # next step
            i_steps += 1
...
def test():
    ...
    if os.path.exists(result_file):
        ...
        all_pred_scores, all_gt_labels, all_pred_fixations, all_gt_fixations, all_toas, all_vids = \
            save_dict['pred_scores'], save_dict['gt_labels'], save_dict[
                'pred_fixations'], save_dict['gt_fixations'], save_dict['toas'], save_dict['vids']
    else:
        # initilize environment
        env = DashCamEnv(cfg.ENV, device=cfg.device)
        env.set_model(pretrained=True, weight_file=cfg.ENV.env_model)
        ...
        agent.set_status('eval')
        with torch.no_grad():
            all_pred_scores, all_gt_labels, all_pred_fixations, all_gt_fixations, all_toas, all_vids = test_all(
                testdata_loader, env, agent)
        np.savez(result_file[:-4], pred_scores=all_pred_scores, gt_labels=all_gt_labels,
                 pred_fixations=all_pred_fixations, gt_fixations=all_gt_fixations, toas=all_toas, vids=all_vids)
    ...
    mTTA = evaluate_earliness(
        all_pred_scores, all_gt_labels, all_toas, fps=FPS, thresh=0.5)
    print("\n[Earliness] mTTA@0.5 = %.4f seconds." % (mTTA))

    AP, p05, r05 = evaluation_accident_new(
        all_pred_scores, all_gt_labels, all_toas, fps=FPS)
    print("[Correctness] AP = %.4f, precision@0.5 = %.4f, recall@0.5 = %.4f" %
          (AP, p05, r05))

    isRandom = True if cfg.baseline == 'random' else False
    AUC_video, AUC_frame = evaluation_auc_scores(
        all_pred_scores, all_gt_labels, all_toas, FPS, video_len=5, pos_only=True, random=isRandom)
