...
def scales_to_point(scales, image_size, input_size):
    ...
    if rows_rate > cols_rate:
        ...
        r = torch.clamp_max(
            input_size[0] / 2.0 * (1 - scale_y), input_size[0] - 1)
        ...
        c = torch.clamp_max(
            input_size[1] / 2.0 * (1 + scale_x), input_size[1] - 1)
        r = r + (input_size[0] - new_rows) // 2
    point = torch.cat((c.unsqueeze(1), r.unsqueeze(1)),
                      dim=1)  # (B, 2): (x, y)
    return point
    ...
    def __call__(self, coords):
        ...
        for i, fixpt in enumerate(coords):
            if fixpt[0] > 0 and fixpt[1] > 0:
                new_coords[i] = padding_point(
                    fixpt, self.img_shape, shape_r, shape_c)
        return new_coords
