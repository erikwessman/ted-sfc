...
def find_valid_sequences(tuples_list):
    ...
    for i in range(1, len(tuples_list)):
        ...
        if (
            current_tuple[0] >= prev_tuple[0]
            and abs(current_tuple[1] - prev_tuple[1]) <= MAX_GAP_S
        ):
...
def get_sequence(cell_values, required_cell_subsets):
    """ """
    path = []
...
def detect_event(
    cell_values, required_cell_subsets
    ...
    """
    sequence_left = get_sequence(cell_values, required_cell_subsets)
    sequence_right = get_sequence(cell_values[::-1], required_cell_subsets)
...
def main(data_path: str, config_path: str):
    ...
    for _, video_id, tqdm_obj in helper.traverse_videos(data_path):
        ...
        event_window, scenario_type = detect_event(cell_values, required_cell_subsets)
