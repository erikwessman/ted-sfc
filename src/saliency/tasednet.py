...
https://github.com/Cogito2012/DRIVE/blob/master/src/saliency/tasednet.py
"""
import torch
...
class TASED_v2(nn.Module):
    def __init__(self, input_shape):
        ...
        self.maxm2 = nn.MaxPool3d(kernel_size=(
            4, 1, 1), stride=(4, 1, 1), padding=(0, 0, 0))
        self.maxt2 = nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(
            1, 2, 2), padding=(0, 1, 1), return_indices=True)
        self.base2 = nn.Sequential(Mixed_3b(), Mixed_3c(),)
        self.maxp3 = nn.MaxPool3d(kernel_size=(
            3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1))
        self.maxm3 = nn.MaxPool3d(kernel_size=(
            4, 1, 1), stride=(4, 1, 1), padding=(0, 0, 0))
        self.maxt3 = nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(
            1, 2, 2), padding=(0, 1, 1), return_indices=True)
        ...
        self.maxp4 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(
            1, 2, 2), padding=(0, 0, 0), return_indices=True)
        ...
        self.convtsp4 = nn.Sequential(
            ...
            nn.Conv3d(4, 4, kernel_size=(2, 1, 1),
                      stride=(2, 1, 1), bias=False),
            nn.BatchNorm3d(4, eps=1e-3, momentum=0.001, affine=True),
            nn.ReLU(),

            nn.ConvTranspose3d(4, 4, kernel_size=(1, 4, 4), stride=(
                1, 2, 2), padding=(0, 1, 1), bias=False),
    ...
    def forward(self, x, return_bottom=False):
        ...
        z = nn.functional.interpolate(z.unsqueeze(
            1), size=self.output_shape).unsqueeze(1)
        if return_bottom:
            return z, bottom
        return z
