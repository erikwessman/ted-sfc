...
def draw_grid(frame, grid_size, skip_top_rows, skip_bottom_rows, skip_right_columns):
    ...
    end_x = (
        w - (w % grid_size) - skip_right_columns * grid_size
    )  # Adjusted for skipping columns
...
while True:
    ...
    for y in range(
        skip_top_rows * grid_size, h - skip_bottom_rows * grid_size, grid_size
    ):
        for x in range(0, w - skip_right_columns * grid_size, grid_size):
            ...
            if cell_index < 12:
                ...
                if 6 <= cell_index <= 6:
                    ...
                    arrow_color = colors[
                        cell_index
                    ]  # Use the assigned color for other cells
                ...
                if moving_avg is not None:
                    ...
                    clamped_angle_diff = (
                        -angle_diff if (-angle_diff) >= angle_threshold else 0
                    )
                    ...
                    current_frame_differences.append(
                        clamped_angle_diff
                    )  # Append angle difference to list
    ...
    # Draw the grid
    draw_grid(
        frame, grid_size, skip_top_rows=3, skip_bottom_rows=2, skip_right_columns=4
    )
...
with open(csv_filename, "w", newline="") as file:
    ...
    # Write the header
    header = ["frame_id"] + [
        f"cell{i+1}" for i in range(6)
    ]  # Adjust the number of cells if necessary
...
for cell_index in range(12):
    ...
    plt.xticks(
        [0, 177, 223, len(angle_differences[cell_index])],
        ["1", "177", "223", str(len(angle_differences[cell_index]))],
    )
